...
export default function Home() {
  ...
  const getWidth = () => {
    ...
    return listCollapsed ? "w-32" : "w-1/6";
  };
  return (
    <div className="flex h-screen w-full">
      <div
        className={clsx(
          "flex flex-col md:p-4 gap-4 fixed w-1/2 left-0 md:static py-2 px-4 transition-all duration-300 scrollbar-hide bg-accent-dark h-full overflow-auto",
          getWidth(),
        )}
      >
        <NotesList notes={mockNotes} />
      </div>
      <div className="flex flex-col p-8 gap-4 flex-grow"></div>
    </div>
  );
}
